...
namespace PartsKit
{
    ...
    /// https://github.com/nathanpjones/randomColorSharped.git
    /// </summary>
    public static class RandomColor
    {
        ...
        private static readonly Dictionary<ColorScheme, DefinedColor> ColorDictionary =
            new Dictionary<ColorScheme, DefinedColor>();

        private static readonly object LockObj = new object();
        private static System.Random _randomIns = new System.Random();
        ...
        public static Color GetColor(ColorScheme scheme, Luminosity luminosity)
        {
            ...
            var h = PickHue(scheme);

            // Then use H to determine saturation (S)
            var s = PickSaturation(h, luminosity, scheme);

            // Then use S and H to determine brightness (B).
            var b = PickBrightness(h, s, luminosity);
        ...
        public static void Seed(int seed)
        {
            lock (LockObj)
            {
                _randomIns = new System.Random(seed);
            }
        }
        ...
        {
            if (ColorDictionary.TryGetValue(colorInput, out var color))
            {
        ...
        private static int RandomWithin(int lower, int upper)
        {
            lock (LockObj)
            {
                return _randomIns.Next(lower, upper + 1);
            }
        }
        ...
        public static Color HsvToColor(int hue, int saturation, double value)
        {
            ...
            if (Math.Abs(h - 360) < float.Epsilon)
            {
                h = 359;
            }
